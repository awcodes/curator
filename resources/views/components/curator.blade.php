...
>
    ...
    <div class="flex-1 relative flex flex-col md:flex-row overflow-hidden">
        ...
        >
            <x-curator::loading-icon class="w-12 h-12 text-gray-700 dark:text-white" />
        </div> <!--loading -->
            ...
            <ul class="grid grid-cols-3 gap-4 sm:grid-cols-4 md:grid-cols-6 2xl:grid-cols-8">
                ...
                <template x-for="file in files">
                    ...
                    <li x-bind:key="file.id" class="relative aspect-square" x-bind:class="{'opacity-40': selected && selected.id !== file.id }">
                        ...
                        <p x-text="file.title ?? file.name" class="text-xs truncate absolute bottom-0 inset-x-0 px-1 pb-1 pt-4 text-white bg-gradient-to-t from-black/80 to-transparent pointer-events-none"></p>
